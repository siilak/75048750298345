...
class PageFields {
  ...
  private function getAllFields($p) {
    ...
    foreach($p->template->fields as $fld) {
      ...
      if (
        $fldType instanceof \ProcessWire\FieldtypeFieldsetOpen ||
        $this->isNotAllowedField($p, $fldName)
      ) continue;

      if ($fldType instanceof \ProcessWire\FieldtypeImage) {
        $flds = array_merge($this->getImages($p, $fld), $flds);
      } else if ($fldType instanceof \ProcessWire\FieldtypeRepeater) {
