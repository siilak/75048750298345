...
;(function(root, factory) {
    ...
    function Pleier(media, config) {
        ...
        // Trigger events, with pleier instance passed
        ...
        function _showCaptions() {
            ...
            var active = pleier.storage.captionsEnabled;
        ...
        function _setTitle(iframe) {
            ...
            if (_is.string(config.title) && config.title.length) {
                ...
                // Set container label
                pleier.container.setAttribute('aria-label', config.title);
            }
        ...
        function _setupStorage() {
            ...
            if (!value) {
                ...
                // version of pleier. See: https://github.com/Selz/pleier/pull/313
                // Update the key to be JSON
                _updateStorage({volume: parseFloat(value)});
            } else {
                // Assume it's JSON from this or a later version of pleier
                pleier.storage = JSON.parse(value);
            }
        }
        ...
        function _updateStorage(value) {
            ...
            _extend(pleier.storage, value);

            // Update storage
            window.localStorage.setItem(config.storage.key, JSON.stringify(pleier.storage));
        }
        ...
        function _setupEmbed() {
            ...
            switch (pleier.type) {
                case 'youtube':
                    mediaId = _parseYouTubeId(pleier.embedId);
                    break;

                case 'vimeo':
                    mediaId = _parseVimeoId(pleier.embedId);
                    break;
            ...
            pleier.embed = new window.YT.Player(container.id, {
                ...
                events: {
                    ...
                    'onReady': function(event) {
                        ...
                        // Set the tabindex
                        if (pleier.supported.full) {
                            pleier.media.querySelector('iframe').setAttribute('tabindex', '-1');
                        }
                        ...
                        // Trigger timeupdate
                        _triggerEvent(pleier.media, 'durationchange');
                        ...
                        timers.buffering = window.setInterval(function() {
                            ...
                            if (pleier.media.lastBuffered === null || pleier.media.lastBuffered < pleier.media.buffered) {
                                _triggerEvent(pleier.media, 'progress');
                            }

                            // Set last buffer point
                            pleier.media.lastBuffered = pleier.media.buffered;
                    ...
                    'onStateChange': function(event) {
                        ...
                        switch (event.data) {
                            ...
                            case 1:
                                ...
                                if (pleier.media.seeking) {
                                    _triggerEvent(pleier.media, 'seeked');
                                }
                                ...
                                // https://github.com/Selz/pleier/issues/374
                                // https://code.google.com/p/gdata-issues/issues/detail?id=8690
                                if (pleier.media.duration !== instance.getDuration()) {
                                    pleier.media.duration = instance.getDuration();
                                    _triggerEvent(pleier.media, 'durationchange');
                                }
        ...
        function _vimeoReady(mediaId, container) {
            ...
            pleier.embed = new window.Vimeo.Player(container, {
                id:         parseInt(mediaId),
                loop:       config.loop,
            ...
            pleier.embed.on('loaded', function() {
                ...
                if (_is.htmlElement(pleier.embed.element) && pleier.supported.full) {
            ...
            pleier.embed.on('seeked', function() {
                pleier.media.seeking = false;
                _triggerEvent(pleier.media, 'seeked');
                _triggerEvent(pleier.media, 'play');
            });
        ...
        function _seek(input) {
            ...
            if (_inArray(config.types.embed, pleier.type)) {
                ...
                // Trigger seeking
                _triggerEvent(pleier.media, 'seeking');
            }
        ...
        function _toggleFullscreen(event) {
            ...
            if (nativeSupport) {
                ...
                document.body.style.overflow = pleier.isFullscreen ? 'hidden' : '';
            }
        ...
        function _setVolume(volume) {
            ...
            // Toggle muted state
            if (volume === 0) {
                pleier.media.muted = true;
            } else if (pleier.media.muted && volume > 0) {
                _toggleMute();
            }
        }
        ...
        function _toggleCaptions(show) {
            ...
            _updateStorage({captionsEnabled: pleier.captionsEnabled});
        }
        ...
        function _toggleControls(toggle) {
            ...
            var delay = 0,
                ...
                show = toggle,
                loading = _hasClass(pleier.container, config.classes.loading);
        ...
        function _updateSource(source) {
            ...
            _toggleClass(pleier.container, config.classes.ready, false);
            ...
            function setup() {
                ...
                if (pleier.type === 'video' && pleier.videoContainer) {
                    _remove(pleier.videoContainer);
                }

                // Reset class name
                if (pleier.container) {
                    pleier.container.removeAttribute('class');
                }
                ...
                if (_inArray(config.types.html5, pleier.type) || (_inArray(config.types.embed, pleier.type) && !pleier.supported.full)) {
                    // Setup interface
                    _setupInterface();
        ...
        function _controlListeners() {
            ...
            if (config.keyboardShorcuts.focused) {
                ...
                _on(pleier.container, 'keydown keyup', handleKey);
            }
            ...
            function handleKey(event) {
                ...
                function seekByKey() {
                    ...
                    var duration = pleier.media.duration;
                ...
                if (pressed) {
                    ...
                    if (!fullscreen.supportsFullScreen && pleier.isFullscreen && code === 27) {
                        _toggleFullscreen();
                    }
            ...
            // Play
            _proxyListener(pleier.buttons.play, 'click', config.listeners.play, togglePlay);

            // Pause
            _proxyListener(pleier.buttons.pause, 'click', config.listeners.pause, togglePlay);
            ...
            // Captions
            _proxyListener(pleier.buttons.captions, 'click', config.listeners.captions, _toggleCaptions);
        ...
        function _mediaListeners() {
            ...
            _on(pleier.media, 'play pause ended', _checkPlaying);
            ...
            _on(pleier.media, config.events.concat(['keyup', 'keydown']).join(' '), function(event) {
        ...
        function _cancelRequests() {
            ...
            pleier.media.setAttribute('src', config.blankUrl);
        ...
        function _destroy(callback, restore) {
            ...
            switch (pleier.type) {
                ...
                case 'vimeo':
                    ...
                    pleier.embed.unload().then(cleanUp);
            ...
            function cleanUp() {
                ...
                pleier.container.parentNode.replaceChild(original, pleier.container);
        ...
        function _init() {
            ...
            if (!_is.htmlElement(pleier.media)) {
                return;
            }
            ...
            if (tagName === 'div') {
                pleier.type     = media.getAttribute('data-type');
                pleier.embedId  = media.getAttribute('data-video-id');
            ...
            // Wrap media
            pleier.container = _wrap(media, document.createElement('div'));

            // Allow focus to be captured
            pleier.container.setAttribute('tabindex', 0);
        ...
        function _setupInterface() {
            ...
            if (!pleier.supported.full) {
                _warn('Basic support only', pleier.type);
        ...
        api = {
            ...
            getContainer:       function() { return pleier.container },
            getEmbed:           function() { return pleier.embed; },
            getMedia:           function() { return pleier.media; },
            getType:            function() { return pleier.type; },
            getDuration:        _getDuration,
            getCurrentTime:     function() { return pleier.media.currentTime; },
            getVolume:          function() { return pleier.media.volume; },
            isMuted:            function() { return pleier.media.muted; },
            isReady:            function() { return _hasClass(pleier.container, config.classes.ready); },
            isLoading:          function() { return _hasClass(pleier.container, config.classes.loading); },
            isPaused:           function() { return pleier.media.paused; },
            on:                 function(event, callback) { _on(pleier.container, event, callback); return this; },
            play:               _play,
            pause:              _pause,
        ...
        function _ready() {
            ...
            window.setTimeout(function() {
                _triggerEvent(pleier.media, 'ready');
            }, 0);

            // Set class hook on media element
            _toggleClass(pleier.media, defaults.classes.setup, true);

            // Set container class for ready
            _toggleClass(pleier.container, config.classes.ready, true);

            // Store a refernce to instance
            pleier.media.pleier = api;
    ...
    function setup(targets, options) {
        ...
        players.forEach(function(player) {
            ...
            try { data = JSON.parse(element.getAttribute('data-pleier')); }
            catch(e) { }
            ...
            // Create new instance
            var instance = new Pleier(media, config);
            ...
            if (config.debug) {
                ...
                _on(instance.getContainer(), events.join(' '), function(event) {
                    console.log([config.logPrefix, 'event:', event.type].join(' '), event.detail.pleier);
                });
            }
    ...
    function get(container) {
        ...
        if (_is.htmlElement(container)) {
            ...
            Array.prototype.slice.call(elements).forEach(function(element) {
                if (_is.object(element.pleier)) {
                    instances.push(element.pleier);
                }
            });
