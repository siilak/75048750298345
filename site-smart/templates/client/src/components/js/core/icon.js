...
export default function (UIkit) {
    ...
    [
        ...
        init() {
            addClass(this.$el, 'tele2-slidenav');

            if (hasClass(this.$el, 'tele2-slidenav-large')) {
                this.icon += '-large';
            }
        }
    ...
    registerComponent('search-icon', {
        ...
        init() {
            if (hasClass(this.$el, 'tele2-search-icon') && parents(this.$el, '.tele2-search-large').length) {
                this.icon = 'search-large';
            } else if (parents(this.$el, '.tele2-search-navbar').length) {
    ...
    registerComponent('close', {
        ...
        init() {
            this.icon = `close-${hasClass(this.$el, 'tele2-close-large') ? 'large' : 'icon'}`;
        }
